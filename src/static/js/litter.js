angular.module('litter', ['ngStorage']).controller('litterCtrl', function ($scope, $localStorage, $sessionStorage) {
	...
	$scope.isActive = function (bibDB) {
		if (bibDB.entry_key == $scope.$state.openKey) return "active";else return "";
	};
...
$(document).ready(function () {
	...
	// add fields by duplicating last one  
	...
	$("form").on('keyup', '[data-type=ISBN]', function (e) {
		...
		for (var i = 1; i < arr.length && res.length <= maxParts; i++) {
			if (arr[i] == "") break;else if (arr[i] !== undefined) res.push(arr[i]);
		}
		$this.val(function () {
